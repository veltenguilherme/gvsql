...
namespace Persistence.Controllers.Base
{
...

    {
        ...
        private new T Find(Predicate<T> predicate)
        {
            ...
            try { return FindAll(predicate).SingleOrDefault(); } catch (Exception) { return default(T); }
        }
        ...
        protected string Name
        {
            get
            {
                try { return typeof(T).GetCustomAttribute<TableAttribute>().Name; }
                catch { throw new Exception($"{typeof(T)} com nome inválido"); }
            }
        }
        ...
        {
            if (IsDefaultNullableValue(property, obj)) return true;

            try
            {
                var name = property.GetCustomAttribute<Base.CustomAttributes.TypeInfo>().Type.ToString().ToUpper();
            }
            catch { return true; }
            ...
            switch (property.PropertyType.Name)
            {
                case "String":
                    return string.IsNullOrEmpty(Convert.ToString(property.GetValue(obj))) &&
                           !"TEXT".Equals(property.GetCustomAttribute<Base.CustomAttributes.TypeInfo>().Type.ToString().ToUpper());

                case "Byte[]": return property.GetValue(obj) == null;
                default: break;
            }
        ...
        private bool IsDefaultNullableValue(PropertyInfo property, T obj)
        {
            ...
            if (fullName.Contains("System.Nullable`1[[System.Int32") ||
                ...
                fullName.Contains("System.Nullable`1[[System.Guid")) return property.GetValue(obj) == null;
